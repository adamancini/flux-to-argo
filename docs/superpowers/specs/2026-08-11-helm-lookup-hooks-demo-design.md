...
3. **`chart/vendored-widget/`** — a stand-in for a third-party chart we're not allowed to edit, clearly marked `DO NOT EDIT` in every file's header. Same shape of problem (lookup + hook Job), deliberately left broken. Paired with a Kustomize overlay at `cluster/argocd/vendored-widget-kustomize/kustomization.yaml`, using Kustomize's `helmCharts` chart-inflator generator plus a `patches:` block rewriting the Job's `helm.sh/hook*` annotations to `argocd.argoproj.io/hook*` equivalents at render time. Requires `kustomize.buildOptions: --enable-helm` added to the AKP instance's `argocd_cm` (one line in the existing `terraform/01-argocd/main.tf`).

**Known risk:** Kustomize's `helmCharts` generator, especially resolving a chart from a local path rather than a remote repo, is version-sensitive and unverified against whatever Kustomize/Helm versions the AKP-hosted repo-server bundles. The implementation plan spikes this locally (`kustomize build` against the overlay using a matching Kustomize version) before wiring it into a live Application — same "verify empirically, adjust if wrong" posture the guestbook plan already used for the `akp` provider's schema. **Fallback** if it doesn't pan out on the hosted repo-server: check in the patched manifests as a plain YAML-directory ArgoCD source, with a comment explaining what a live post-renderer pipeline would otherwise produce.
...
4. **`task adminapp:vendored`** — deploy `chart/vendored-widget` via the Kustomize overlay, and diff the vendored chart's raw `helm template` output against the ArgoCD Application's actual applied manifests (`argocd app manifests`) to show the hook annotations were rewritten at render time without editing the chart.
...
- **Kustomize `helmCharts` local-chart resolution fails against the AKP repo-server's bundled versions**: fall back to a checked-in, pre-patched manifest directory (see Architecture's Known risk).
...
- `chart/vendored-widget`'s non-idempotent Job logic is intentionally never fixed — only its ArgoCD lifecycle mapping is. That gap is the point of the third chart.
