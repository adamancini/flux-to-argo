...
A subsequent revision restructured the whole script into labeled `BEFORE`/`STEP N`/`AFTER` sections, each printing a one-line Flux HelmRelease snapshot (`flux get helmrelease`) and a terse ArgoCD Application sync/health snapshot (`argocd app get -o json` piped through `jq`), so a human running this live sees exactly how Flux's and ArgoCD's view of the resource change at each transition, not just a wall of `argocd app sync`'s own verbose resource tables.
